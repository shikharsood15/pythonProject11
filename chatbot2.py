...
for intent in ourData["ourIntents"]:
    for pattern in intent["patterns"]:
        ...
        newWords.extend(ournewTkns)# extends the tokens The extend() method adds the specified list elements (or any iterable) to the end of the current list
        documentX.append(pattern)# append all the pattern
        documentY.append(intent["tag"])# append all the tags


    if intent["tag"] not in ourClasses:# add unexisting tags to their respective classes
        ourClasses.append(intent["tag"])# append intent["tag"] to our classes
...
newWords = sorted(set(newWords))# sorting words will act as a vocabulary
...
# bow model
for idx, doc in enumerate(documentX):# enumerate will give the index and values of the document present
    bagOfwords = []
    text = lm.lemmatize(doc.lower())#lemmatize the doc showing in lowercase
    for word in newWords:
        bagOfwords.append(1) if word in text else bagOfwords.append(0)

    outputRow = list(outEmpty)
    outputRow[ourClasses.index(documentY[idx])] = 1# not understood
...
ourNewModel.fit(x, y, epochs=10, verbose=1)
